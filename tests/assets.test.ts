...
describe("art asset manifest", () => {
  ...
  it("provides a descriptor and local fallback for every runtime key", () => {
  ...
  it("maps every configured mine item to a distinguishable texture key", () => {
    const itemTypes = Object.keys(ITEM_CONFIGS) as Array<keyof typeof ITEM_CONFIGS>;
    expect(Object.keys(ITEM_TEXTURE_KEYS).sort()).toEqual(itemTypes.sort());
    expect(new Set(Object.values(ITEM_TEXTURE_KEYS)).size).toBe(itemTypes.length);
